...
list_of_images_to_be_blur = get_all_images(images_dir)

output_directory = "output/"

os.makedirs(output_directory, exist_ok=True)

for image in list_of_images_to_be_blur:
    image_path = os.path.join(image_base_path, image)
    load_image = cv2.imread(image_path)
    
    filename = os.path.basename(image_path)

    name, extension = os.path.join(output_directory, f"{name}_blurred{extension}")
